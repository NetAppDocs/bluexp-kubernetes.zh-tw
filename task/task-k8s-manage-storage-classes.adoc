...
+
image:screenshot-k8s-storage-economy.png["在設定K8s儲存類別時、顯示區塊儲存的fstype,選項的快照。"]

.. 對於*區塊*或*檔案系統*儲存設備、您可以選擇啟用儲存經濟效益。
+
image:screenshot-k8s-storage-fstype.png["在設定區塊或檔案K8s儲存類別時、顯示「儲存經濟」選項的快照。"]



. 選取Volume擴充、Volume集區和預設儲存類別的選項。單擊 * 下一步 * 。
